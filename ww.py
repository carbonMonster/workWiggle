...
# IDLE_TIME_THRESHOLD_SECONDS = 400  # Call discord_wiggle if system idle > 300 seconds
# DISCORD_IDLE_THRESHOLD_SECONDS = 500  # Call discord_wiggle if Discord idle > 500 seconds
# DELAY_MEAN_SECONDS = 250      # Mean delay before discord_wiggle
# DELAY_STD_SECONDS = 135       # Std dev for delay


IDLE_TIME_THRESHOLD_SECONDS = 4  # Call discord_wiggle if system idle > 300 seconds
DISCORD_IDLE_THRESHOLD_SECONDS = 5  # Call discord_wiggle if Discord idle > 500 seconds
DELAY_MEAN_SECONDS = 2      # Mean delay before discord_wiggle
DELAY_STD_SECONDS = 1       # Std dev for delay
    ...
    try:
        ...
        try:
            ...
            channel_urls = [
                ...
                "discord://-/channels/@me/1105197989192011897",
...
if __name__ == "__main__":
    try:
        ...
        # Terminate existing tailing processes
        for proc in psutil.process_iter(['name', 'cmdline']):
            try:
                if proc.info['name'].lower() in ['powershell.exe', 'python.exe', 'pythonw.exe']:
                    cmdline = proc.info.get('cmdline', [])
                    if any('discord_output.txt' in str(arg) or 'discord_temp_output.txt' in str(arg) for arg in cmdline):
                        proc.terminate()
                        proc.wait(timeout=5)
                        log_message(f"Terminated tailing process PID: {proc.pid}")
            except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.TimeoutExpired) as e:
                log_message(f"Error terminating process: {str(e)}")
        
        # Clear discord_output.txt
        output_file = "discord_output.txt"
        with open(output_file, 'w', encoding='utf-8') as f:
            pass
        log_message(f"Cleared {output_file}")
        ...
        root = tk.Tk()
        root.attributes('-topmost', True)  # Corrected from '- Impacts'
        root.withdraw()
